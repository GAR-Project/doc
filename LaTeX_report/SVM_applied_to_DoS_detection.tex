...
\section{Mininet's Internals}
	...
	\begin{figure}
		\centering
		\includegraphics[scale = 1]{emulation.png}
		\caption{A videogame portraying a flight emulator}
		\label{f:emulator}
	\end{figure}

	With this little detail out of the way we could ask ourselves. Does \textbf{mininet} emulate or simulate a network?. It is a network \textbf{emulator}, here's why. Mininet reserves system resources for each node in the \textbf{emulated} network. You might think these nodes are "just" VMs or virtualized containers but... they're not. That solution would have many advantages but it wouldn't scale to be able to \textbf{emulate} large networks or huge amounts of traffic as it would exhaust the host system's resources... The Mininet developers then chose to \textbf{exclusively virtualize} what was necessary to carry out the desired \textbf{network emulation}.
	\newpar
	...
	\subsection{Network Namespaces}
		...
		\begin{figure}
			\centering
			\includegraphics[scale = 1]{vegeta.png}
		...
		\begin{figure}
			\centering
			\includegraphics[scale = 1]{x_htop.png}
...
\section{Mininet's Internals (II)}
	...
	sudo ip netns list
		...
		\begin{figure}
			\centering
			\includegraphics[scale = 1]{netns_list.png}
			\label{f:netns_list}
		\end{figure}
		...
		\begin{figure}
			\centering
			\includegraphics[scale = 1]{calm_tux.jpg}
			\label{f:tux}
		\end{figure}
	...
	sudo starce ip netns list
			...
			\begin{figure}
				\centering
				\includegraphics[scale = 1]{sys_trace.jpg}
				\label{f:sys_trace}
			\end{figure}
	...
	+++ exited with 0 +++
		...
		\subsubsection{So where are Mininet's Network Namespaces located?}
			...
			\begin{itemize}
				...
				\item As long as you have opened a file descriptor to the namespace file. (}/proc/{pid}/ns/{type_namespace}\texttt{)
				\item As long as there is a bind mount of the file (}/proc/{pid}/ns/{namespace type}\texttt{) of the namespace in question.
			\end{itemize}

			If none of these conditions are met, the namespace in question is \textbf{deleted}. If it is a }net' type namespace (a.k.a Network Namespace) those interfaces that are in the disappearing namespace will return to the default namespace. Once we understand this concept, we must think about the nature of the Network namespaces that Mininet creates.
			\newpar
		...
		\subsubsection{Just a hypothesis?}
			...
			Let's set the scenario up if we haven't set it up before, list all processes, and filter by the name of \textit{mininet}. Let's see what we find :grimacing:.
	...
	sudo ps aux | grep mininet
			...
			\begin{figure}
				\centering
				\includegraphics[scale = 1]{netns_procs.png}
				\label{f:netns_procs}
			\end{figure}
			...
			\nerwpar
			If we inspect the \texttt{/proc/{pid}/ns/net} file for each process we can see which ones are in a different network namespace depending on the value of the inode. For example, let's check the processes associated to hosts 1 and 2 can be seen in figures \ref{host1_procs} and \ref{host2_procs} respectively.

			\begin{figure}
				\centering
				\includegraphics[scale = 1]{host1_procs.png}
				\label{f:host1_procs}
			\end{figure}

			\begin{figure}
				\centering
				\includegraphics[scale = 1]{host2_procs.png}
				\label{f:host2_procs}
			\end{figure}
	...
	nsenter --target <pid> --net {Command}
			...
			The output of the above command for hosts 1 and 2 can be seen in figures \ref{f:host1_intf} and \ref{host2_intf} respectively.

			\begin{figure}
				\centering
				\includegraphics[scale = 1]{host1_intf.png}
				\label{f:host1_intf}
			\end{figure}

			\begin{figure}
				\centering
				\includegraphics[scale = 1]{host2_intf.png}
				\label{f:host2_intf}
			\end{figure}
			...
			\newpar
			Additionally we can corroborate our hypothesis by changing the "\textit{verbosity}" of our script, where we build the whole scenario topology, \href{https://github.com/GAR-Project/project/blob/master/src/scenario_basic.py}{\texttt{src/scenario_basic.py}}, we can change the level of \texttt{info} to \texttt{debug}, and launch the script again.
	...
	if __name__ == '__main__':
		...
		scenario_basic()
			...
			\begin{figure}
				\centering
				\includegraphics[scale = 1]{debug.png}
				\label{f:debug}
			\end{figure}
		...
		\subsubsection{So, is it possible to use \texttt{iproute2} with Mininet?}
			...
			\newpar
			So, there is no solution :unamused:? Well, almost everything has a solution, it depends on us to how far we want to go to fix things. Let's see how we can enable the Network namespace of \textbf{Host1} to be visible for \texttt{ip netns}.
			\newpar
	...
	sudo ps aux | grep mininet | grep h1
			...
			\begin{figure}
				\centering
				\includegraphics[scale = 1]{h1_pid.png}
				\label{f:h1_pid}
			\end{figure}
	...
	sudo ln -sfT /proc/<PID>/ns/net /var/run/netns/h1
			...
			\begin{figure}
				\centering
				\includegraphics[scale = 1]{lnk.png}
				\label{f:lnk}
			\end{figure}
			...
			\begin{figure}
				\centering
				\includegraphics[scale = 1]{it_works.png}
				\label{f:it_works}
			\end{figure}
			...
			\begin{figure}
				\centering
				\includegraphics[scale = 1]{iproute2_fixed.png}
			...
			\begin{figure}
				\centering
				\includegraphics[scale = 1]{shutting_down.png}
				\label{f:shutting_down}
			\end{figure}

			\begin{figure}
				\centering
				\includegraphics[scale = 1]{broken_softlink.png}
	...
	\subsection{The Big Picture}
		...
		\begin{figure}
			\centering
			\includegraphics[scale = 1]{switches_ln.png}
			\label{f:switches}
		\end{figure}
		...
		\begin{figure}
			\centering
			\includegraphics[scale = 1]{scenario_kernel.png}
			\label{f:scenario_kernel}
		\end{figure}

		So, to run \texttt{telegraf} only on the switches we would just launch it on the default network namespace! This can be done with a single \texttt{telegraf} process since the useful interfaces are all in the same Network namespace :relaxed: .
